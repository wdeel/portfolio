...
class IRCTestManager(object):
    ...
    def check_server(self, server, configuration):
        ...
        if 'channels' in configuration:
            problems += self.find_problems_with_server_channels(server.servername, configuration['channels'])
    ...
    def find_problems_with_server_channels(self, servername, channels):
        ...
        missing_from_server = self.diff(channels, self.servers[servername].channels.keys())
        if missing_from_server:
            problems += "%s: Missing from channels: %s\n" % (servername, ", ".join(missing_from_server))

        extra_in_server = self.diff(self.servers[servername].channels.keys(), channels)
        if extra_in_server:
            problems += "%s: Extra in channels: %s\n" % (servername, ", ".join(extra_in_server))

        for channel in channels:
            problems += self.find_problems_with_server_channel(servername, channel, channels[channel])
        return problems
    ...
    def find_problems_with_server_channel(self, servername, channel_name, channel_data):
        ...
        if channel_name in self.servers[servername].channels:
            ...
            missing_from_server = self.diff(channel_data['users'], server_channel.users)
            if missing_from_server:
                problems += "%s: Missing users in channel %s: %s\n" % (servername, channel_name, ", ".join(missing_from_server))

            extra_in_server = self.diff(server_channel.users, channel_data['users'])
            if extra_in_server:
    ...
    def check_client(self, client, configuration):
        ...
        missing_from_client = self.diff(configuration['channels'], client.channels)
        ...
        for channelname in self.intersect(client.channels.keys(), configuration["channels"].keys()):
            problems += self.find_problems_with_client_channel(client.nick, channelname, client.channels[channelname], configuration["channels"][channelname])

        # Check for issues in the printed messages
        missing_from_client = self.diff(configuration['printed_messages'], client.printed_messages)
        if missing_from_client:
            problems += "%s: Missing messages: %s\n" % (client.nick, ", ".join(missing_from_client))

        extra_in_client = self.diff(client.printed_messages, configuration['printed_messages'])
        if extra_in_client:
